...
OUTPUT_FILE = "nison_expert_signals_pro.csv"
...
# --- 2. TECHNICAL INDICATORS (Updated with Pro Signals) ---
...
def get_trend(row):
    # Only consider it a downtrend if price is below SMA20
...
def check_patterns(ticker, df):
    ...
    if patterns:
        ...
        macd_buy = "YES" if (c0['MACD_Hist'] > 0 and c1['MACD_Hist'] < 0) else "NO"
        
        # Stochastic Cross: K crosses D while < 20 (Oversold)
        stoch_buy = "YES" if (c0['Stoch_K'] > c0['Stoch_D'] and 
                              c1['Stoch_K'] < c1['Stoch_D'] and 
                              c0['Stoch_K'] < 20) else "NO"
        ...
        return {
            ...
            # New Pro Signals
            "MACD Signal": macd_buy,
            "Stoch Signal": stoch_buy
        }
    return None

def main():
    print("--- NISON EXPERT ADVISOR v2 PRO (Full Data) ---")
    ...
    if len(tickers) == 1:
        ...
        for ticker in avail:
            try:
                ...
                if res:
                    print(f"FOUND: {ticker} -> {res['Rating']} ({res['Pattern']}) | MACD: {res['MACD Signal']} | Stoch: {res['Stoch Signal']}")
                    results.append(res)
            except: continue
    ...
    if results:
        ...
        print(df_res[['Ticker', 'Price', 'Pattern', 'Rating', 'MACD Signal', 'Stoch Signal']].head(10).to_string(index=False))
    else:
