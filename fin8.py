...
def check_divergence(df):
    ...
    # Logic: Lowest Price is recent (<3 days), Lowest RSI is old (>5 days)
...
# --- 4. MAIN ENGINE ---
def main():
    print("--- NISON EXPERT SIGNALS (Complete Mode) ---")
    ...
    for ticker in avail:
        try:
            ...
            # 3. Always Save Result with DATE
            c0 = df_t.iloc[-1]
            ...
            res = {
                ...
                "RSI": round(c0['RSI'], 1),
                "Stoch": round(c0['Stoch_K'], 1),
                "Rel Vol": round(vol_ratio, 1)
            }
            results.append(res)
            
            if patterns != "None":
                print(f"[{signal_date}] Match: {ticker} -> {patterns}")
    ...
    if results:
        ...
        cols = ['Date', 'Ticker', 'Price', 'Patterns', 'Daily Trend', 'Bullish Divergence', 'Volatility Squeeze', 'PTJ Status', 'Golden Cross', 'RSI', 'Stoch', 'Rel Vol']
